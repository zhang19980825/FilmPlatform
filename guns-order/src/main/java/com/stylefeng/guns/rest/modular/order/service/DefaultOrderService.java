...
import com.stylefeng.guns.core.uuid.UUIDUtil;
...
public class DefaultOrderService implements OrderServiceAPI {
    ...
    //创建新订单
    @Override
    public OrderServiceVO saveOrderInfo(Integer fieldId, String soldSeats, String seatsName, Integer userId) {
        //编号
        ...
        if(insert>0){
            // 返回查询结果
            OrderServiceVO orderVO = moocOrderTMapper.getOrderInfoById(uuid);
    ...
    @Override
    public Page<OrderServiceVO> getOrderByUserId(Integer userId,Page<OrderServiceVO> page) {
        Page<OrderServiceVO> result = new Page<>();
        if(userId==null){
            log.error("订单查询业务失败，用户编号未传入");
            return null;
        }else {
            List<OrderServiceVO> ordersByUserId = moocOrderTMapper.getOrdersByUserId(userId,page);
    ...
    @Override
    public OrderServiceVO getOrderInfoById(String orderId) {
        OrderServiceVO orderServiceVO=moocOrderTMapper.getOrderInfoById(orderId);
